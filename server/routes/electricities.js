...
let Electricity = require("../model/Electricity");
...
router.route("/add").post((req, res) => {
  ...
  const totalEnergyProduced =
    pannelSize * intensity * irradiance * conversionEfficiency;
...
router.get("/all", async (req, res) => {
  const electricities = await Electricity.find();
  res.json({
    success: true,
    existingElectricities: electricities,
  });
...
router.route("/delete/:id").delete(async (req, res) => {
  ...
  await Electricity.findByIdAndDelete(vId)
    .then(() => {
    ...
    .catch((err) => {
      ...
      res
        .status(500)
        .send({ status: "Error with delete the details", error: err.message });
    });
});
