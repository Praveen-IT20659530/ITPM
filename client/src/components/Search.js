...
export default class Search extends Component {
  ...
  retriveElectricities() {
    axios.get("http://localhost:5000/electricity/all").then((res) => {
      if (res.data.success) {
        this.setState({
  ...
  filterContent(electricities, searchTerm) {
    const results = electricities.filter((electricities) =>
      electricities.company.toLowerCase().includes(searchTerm)
    );
  ...
  handleTextSearch = (e) => {
    ...
    axios.get("http://localhost:8070/electricity/all").then((res) => {
      if (res.data.success) {
        this.filterContent(res.data.existingElectricities, searchTerm);
      }
    });
  };
    ...
    return (
      <div className="container" class="p-3 mb-2 bg-dark text-white">
        ...
        <div className="col-lg-9 mt-2 mb-2">
          <h3>
            {" "}
            <strong> Search The Company Name Here </strong>
          </h3>
        </div>
